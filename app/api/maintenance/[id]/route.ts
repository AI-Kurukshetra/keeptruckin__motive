...
import { companyQuerySchema, maintenanceUpdateSchema } from "@/lib/validations/api";
...
export async function PATCH(
  ...
  if (!parsedParams.success) return fail("Invalid maintenance id", 400, parsedParams.error.flatten());
  ...
  const parsed = await parseJsonBody(request, maintenanceUpdateSchema);
  ...
  const { data, error } = await supabase
    ...
    .update({
      ...
      odometer_miles_at_service: parsed.data.odometerMilesAtService,
  ...
  if (error) return fail("Failed to update maintenance record", 500, error.message);
  return ok(data);
}
