...
(async () => {
  ...
  const registerEmail = `qa${shortId}@example.com`;
  const fallbackEmail = `qae2e${shortId}@example.com`;
  ...
  await attemptStep('1. Register a new user', async () => {
    await page.goto('http://localhost:3000/register', { waitUntil: 'networkidle' });
    await page.getByTestId('register-email').fill(registerEmail);
    ...
    if (!url.includes('/login?message=account_created')) {
      throw new Error(`registration failed: ${err || url}`);
    }

    return `registered ${registerEmail}`;
  });

  await attemptStep('2. Complete company onboarding', async () => {
    let loginUrl = await loginWith(registerEmail);
    let loginEmail = registerEmail;

    if (!loginUrl.includes('/dashboard')) {
      const ready = await createFallbackUser(env, fallbackEmail, password, report.notes);
      if (!ready) throw new Error(`login failed and fallback user create failed (${loginUrl})`);
      loginEmail = fallbackEmail;
      loginUrl = await loginWith(loginEmail);
    }
    ...
    if (await page.getByTestId('create-company-form').count()) {
      await page.getByTestId('company-name').fill(`QA Fleet ${shortId}`);
      await page.getByTestId('company-dot-number').fill(`DOT${shortId}`);
      await page.getByTestId('company-fleet-size').fill('5');
      await Promise.all([
        page.waitForLoadState('networkidle'),
        page.getByTestId('create-company-submit').click(),
      ]);
    }

    return `authenticated as ${loginEmail}`;
  });
