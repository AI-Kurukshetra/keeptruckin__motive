...
describe("alerts route", () => {
  ...
  it("returns 401 when unauthenticated", async () => {
    ...
    const response = await GET(
      new Request("http://localhost/api/alerts?companyId=6b2be5a3-a8db-4793-a8a7-4e5ff20f8d4c")
    );

    expect(response.status).toBe(401);
  });

  it("returns 400 for invalid alert payload", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/alerts", {
        ...
        body: JSON.stringify({ companyId: "bad-id", title: "" }),
      })
    );
